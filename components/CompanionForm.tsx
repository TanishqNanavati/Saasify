...
const formSchema = z.object({
    ...
    duration: z.number().min(1, { message: 'Duration is required.' }) // Explicitly define as number
});
...
const CompanionForm = ()=>{
    ...
    const onSubmit = async(values : z.infer<typeof formSchema>) =>{
      ...
      if(companion) redirect(`/companiens/${companion.id}`);
      else{
    ...
    return(
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Button
          type="submit"
          className="w-full bg-[#6c47ff] text-white font-medium rounded-full h-12 transition-all duration-300 transform hover:scale-105 hover:bg-[#5b3de0] active:scale-95 cursor-pointer shadow-md hover:shadow-lg"
        >
